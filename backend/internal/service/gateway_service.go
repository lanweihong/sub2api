...
type RecordUsageInput struct {
	...
	// ParsedRequest 用于 Claude Max 策略（可选）
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
		...
		persistPayloadAuditIfNeeded(ctx, "service.gateway", usageLog.RequestID, s.payloadRepo, usageLog.ID, input.RequestPayload, input.ResponsePayload, input.RequestTruncated, input.ResponseTruncated)
