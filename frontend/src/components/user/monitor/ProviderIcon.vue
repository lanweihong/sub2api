...
type IconProvider = 'openai' | 'anthropic' | 'gemini'
...
const PROVIDER_ICONS: Record<IconProvider, IconData> = {
  openai: {
    paths: [
...
function isIconProvider(provider: string): provider is IconProvider {
  return provider === 'openai' || provider === 'anthropic' || provider === 'gemini'
}

const iconInfo = computed<IconData | null>(() => {
  if (isAnthropicCompatPlatform(props.provider)) {
    return PROVIDER_ICONS.anthropic
  }
  if (isIconProvider(props.provider)) {
    return PROVIDER_ICONS[props.provider]
  }
  return null
})
